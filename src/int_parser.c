...
int *_tms_get_int_op_indexes(char *local_expr, tms_int_subexpr *S, int s_i)
{
    ...
    {
        tms_save_error(TMS_INT_PARSER, INTERNAL_ERROR, EH_FATAL, local_expr, solve_start);
        return NULL;
    }
    ...
    for (int i = solve_start; i <= solve_end; ++i)
    {
        ...
        {
            tms_save_error(TMS_INT_PARSER, SYNTAX_ERROR, EH_FATAL, local_expr, i);
            free(operator_index);
            return NULL;
        }
    }
...
int _tms_init_int_nodes(char *local_expr, tms_int_expr *M, int s_i, int *operator_index)
{
    ...
    if (op_count > 0)
    {
        ...
        {
            tms_save_error(TMS_INT_PARSER, RIGHT_OP_MISSING, EH_FATAL, local_expr, operator_index[op_count - 1]);
            return -1;
        }
...
int _tms_set_all_int_operands(char *local_expr, tms_int_expr *M, int s_i)
{
    ...
    if (NB[0].operator_index == S[s_i].solve_start)
    {
        ...
        {
            tms_save_error(TMS_INT_PARSER, SYNTAX_ERROR, EH_FATAL, local_expr, NB[0].operator_index);
            return -1;
        }
    }
    else
...
int _tms_set_int_operand(char *expr, tms_int_expr *M, tms_int_op_node *N, int op_start, int s_i, char operand)
{
    ...
    case 'l':
        ...
        break;
    default:
        tms_save_error(TMS_INT_PARSER, INTERNAL_ERROR, EH_FATAL, expr, N->operator_index);
        return -1;
    }
    ...
    if (status == -1)
    {
        ...
        if (status != 0)
        {
            if (tms_get_error_count(TMS_INT_PARSER, EH_FATAL) == 0)
                tms_save_error(TMS_INT_PARSER, SYNTAX_ERROR, EH_FATAL, expr, op_start);
            return -1;
        }
    }
    else
